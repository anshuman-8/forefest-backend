...
const name= yup.string().min(2,'Password must be at least 2 characters').matches(/^[a-zA-Z0-9]+$/,'Name must be alphanumeric').required();

const email= yup.string().email("Email must be a valid email").required();

const password= yup.string().required().min(6,'Password must be at least 6 characters').max(20,'Password must be at most 20 characters');
